...
class RnnDocReader(nn.Module):
    ...
    def __init__(self, opt, padding_idx=0, embedding=None):
        ...
        question_input_size = opt['embedding_dim'] + opt['num_word_features']
        ...
        doc_input_size = question_input_size + opt['num_features']
        if opt['use_qemb']:
            doc_input_size += question_input_size
    ...
    def forward(self, x1, x1_wf, x1_f, x1_mask, x2, x2_wf, x2_mask):
        """Inputs:
        ...
        x1_emb = torch.cat([self.embedding(x1), x1_wf], 2)
        x2_emb = torch.cat([self.embedding(x2), x2_wf], 2)
